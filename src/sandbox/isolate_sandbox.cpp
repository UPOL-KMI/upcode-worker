...
char **isolate_sandbox::isolate_run_args(const std::string &binary, const std::vector<std::string> &arguments)
{
	...
	vargs.push_back("--cg");
	// No --cg-timing: Isolate 2.0 removed the switch because in control-group mode that timing is
	// now always used. Passing it makes isolate exit with a usage error before running anything.
